...
- Implemented so far: app shell, SwiftData persistence with seeded categories, local media storage, closet capture/import with native clothing foreground extraction, avatar onboarding with silhouette generation, the try-on studio, save-look persistence, and lookbook detail/reopen.
- Remaining for Milestone 1: privacy/settings affordances including local data deletion, wardrobe item replace-photo, avatar body-shape adjustment controls, and a physical-device/TestFlight readiness pass.
